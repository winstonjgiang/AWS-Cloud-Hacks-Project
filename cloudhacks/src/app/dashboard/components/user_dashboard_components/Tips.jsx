...
export default function TipsPanel({ categoryData, recurringEvents, aiTips }) {
  ...
  const totalEvents =
    categoryData?.reduce((sum, cat) => sum + cat.value[1], 0) || 0;
  const recurringCount = recurringEvents
    ? Object.keys(recurringEvents).length
    : 0;
  ...
  const tips = [
    {
      ...
      tip:
        aiTips?.Academic ||
        `You have ${academicEvents} academic events this week, averaging ${(
          academicHours / 7
        ).toFixed(1)} hours per day.`,
      color: "blue.400",
    ...
    },
    {
      ...
      tip:
        aiTips?.Exercise ||
        `With ${exerciseEvents} exercise events scheduled, you're dedicating time to your physical health.`,
      color: "green.400",
    ...
    },
    {
      ...
      tip:
        aiTips?.Personal ||
        `You've allocated ${(socialHours / 7).toFixed(
          1
        )} hours per day for personal and social activities.`,
      color: "yellow.400",
    ...
    },
    {
      ...
      tip:
        aiTips?.General ||
        `Your week includes ${totalEvents} total events with ${recurringCount} recurring activities. A mix of routine and flexibility helps create a sustainable and productive schedule.`,
      color: iconColor,
      ...
      {tips.map((tip, index) => (
        ...
        >
          <Flex align="start" gap={3}>
            <Icon as={tip.icon} color={tip.color} boxSize={5} mt={1} />
            <Box>
