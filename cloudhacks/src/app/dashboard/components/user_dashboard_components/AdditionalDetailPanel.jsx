...
export default function AdditionalDetailPanel({
  categoryData,
  recurringEvents,
}) {
  return (
    <Stack spacing={8}>
        ...
        <Grid templateColumns={{ base: "1fr", md: "repeat(3, 1fr)" }} gap={4}>
          {categoryData.map((category, index) => {
            ...
            const totalEvents = categoryData.reduce(
              (sum, cat) => sum + Number(cat.value[1]),
              0
            );
            const eventPercentage = Math.round(
              (eventCount / totalEvents) * 100
            );
            ...
            return (
              ...
              >
                ...
                {/* End of summary of category data */}
              </Box>
            );
          })}
        </Grid>
      ...
      <Box>
        ...
        >
          <Stack spacing={6} pr={2}>
            {" "}
            {/* Added right padding to content to prevent overlap with scrollbar */}
